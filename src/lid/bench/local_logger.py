...
class LocalResultsLogger:
    ...
    def generate_report(
        ...
        if successful:
            ...
            lines.append("| # | run_name | sps | mem_mb | acc | wall_sec | energy_mj | W&B |")
            lines.append("|---|----------|-----|--------|------|----------|-----------|-----|")
        ...
        if failed:
            lines.append("## Errors\n")
            for r in failed:
                lines.append(f"- **{r.get('strategy', '?')}**: {r.get('error', 'Unknown error')}")
            lines.append("")
        else:
        ...
        lines.append("---\n")
        lines.append(
            f"*Generated by lid-bench on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*\n"
        )
    ...
    def finalize(
        ...
        self.generate_report(results, grid, config_path)
        self._update_latest_symlink()

        print(f"\nLocal results saved to: {self.step_dir}/")
        print("  benchmark_results.csv  (step results)")
        print("  config.yaml            (experiment config)")
        print("  platform.json          (system info)")
        print("  REPORT.md              (auto-generated report)")
